...
/**
 * @param {{ source: any, collection: string, standard?: unknown[], ikoner?: readonly string[], namn?: string }} config
 */
export function createCatalogSource(config) {
   ...
   */
  const { source, collection, standard = [], ikoner, namn = "katalog" } = config ?? /** @type {any} */ ({});
   ...
   */
  const reserv = validateKatalog(standard, { ikoner, katalog: `${namn} (standardvärden)` });
  ...
  return {
    ...
    async las() {
      try {
        ...
        const kategorier = validateKatalog(Array.isArray(rader) ? rader : [], { ikoner, katalog: namn });
