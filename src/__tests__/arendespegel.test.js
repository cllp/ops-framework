...
import { skapaArendespegel } from "../nod/arendespegel.js";
...
describe("skapaArendespegel, konfigurationen", () => {
  it("kräver etikett, eftersom ett saknat urval ser ut som att speglingen fungerar", () => {
    ...
    expect(() => skapaArendespegel({ agare: "a", repo: "r", etikett: "" })).toThrow(/etikett krävs/);
    expect(() => skapaArendespegel({ agare: "a", repo: "", etikett: "e" })).toThrow(/repo krävs/);
    expect(() => skapaArendespegel({ agare: "", repo: "r", etikett: "e" })).toThrow(/agare krävs/);
  });
...
describe("hamta", () => {
  it("kastar vid fel svar i stället för att svara med en tom lista", async () => {
    ...
    // trasigt anrop.
    const { hamtare } = svarar("Bad credentials", { ok: false, status: 403 });
    const spegel = skapaArendespegel({ ...KONFIG, hamtare });
    await expect(spegel.hamta("t")).rejects.toThrow(/403/);
    await expect(spegel.hamta("t")).rejects.toThrow(/Bad credentials/);
  });
  ...
  it("kräver en token", async () => {
    const spegel = skapaArendespegel(KONFIG);
    await expect(spegel.hamta("")).rejects.toThrow(/token krävs/);
  });

  it("skickar etiketten och bara öppna ärenden", async () => {
    const { hamtare, anrop } = svarar([]);
    const spegel = skapaArendespegel({ ...KONFIG, hamtare });
    await spegel.hamta("hemlig");
...
describe("tillFlode", () => {
  ...
  it("filtrerar bort pull requests", () => {
    ...
    const flode = spegel.tillFlode([arende({ number: 1 }), arende({ number: 2, pull_request: { url: "x" } })], { nu });
  ...
  it("sorterar på senast ändrad, med numret som andrahandsnyckel", () => {
    ...
    expect(spegel.tillFlode(rader, { nu }).items.map((p) => p.number)).toEqual([11, 12, 10]);
  });
  ...
  it("ger tom sammanfattning utan en sammanfattningsfunktion, aldrig första raden", () => {
    ...
    const utan = spegel.tillFlode([arende({ body: "## Rubrik\n\nEn mening om något." })], { nu });
    expect(utan.items[0].summary).toBe("");

    const med = skapaArendespegel({ ...KONFIG, sammanfattning: (t) => t.split("\n").pop() || "" });
  ...
  it("läser etiketter både som objekt och som strängar", () => {
    ...
    const flode = spegel.tillFlode([arende({ labels: [{ name: "drift" }, "bradskande"] })], { nu });
    expect(flode.items[0].labels).toEqual(["drift", "bradskande"]);
  });

  it("bygger en länk när GitHub inte skickar en", () => {
    const flode = spegel.tillFlode([arende({ number: 42, html_url: undefined })], { nu });
  ...
  it("skriver inget live-fält", () => {
    ...
    const flode = spegel.tillFlode([arende({})], { nu });
    expect("live" in flode).toBe(false);
  });

  it("appens extra fält följer med per post", () => {
    const med = skapaArendespegel({
      ...KONFIG,
      extraFalt: (post) => ({ roller: post.number === 1 ? ["manniska"] : ["agent"] }),
    });
  ...
  it("tål en tom och en saknad lista", () => {
    expect(spegel.tillFlode([], { nu }).items).toEqual([]);
    expect(spegel.tillFlode(/** @type {any} */ (null), { nu }).items).toEqual([]);
  });
});
...
describe("spegeln och läsaren hänger ihop", () => {
  it("det spegeln skriver kan läsaren läsa", () => {
    ...
    // lista i vyn, alltså "inga uppgifter", vilket är felklassen `lasArendeflode`
  ...
  it("och även efter en tur genom JSON", () => {
    const spegel = skapaArendespegel(KONFIG);
    const flode = spegel.tillFlode([arende({ number: 7 })], { nu: () => "2026-09-17" });
    const last = lasArendeflode(JSON.stringify(flode));
    expect(last.fanns).toBe(true);
    expect(last.poster.map((p) => p.number)).toEqual([7]);
  });
});
